...
const rgbaToArray = (string) =>
    string
        .replace(/[^\d,]/g, '')
        .split(',')
        .map((x, i) => (i < 3 ? parseInt(x) / 255 : parseInt(x)))
...
const lerpRgba = (col1, col2, factor = 0.5) =>
    col1.map((x, i) => x + factor * (col2[i] - x))
...
const getCoord = (el) => {
    ...
    attributes.easing = el.getAttribute('easing')
        ? el.getAttribute('easing')
        : 'easeInOutQuart'
    attributes.scale = scale
    return {
        x: normX(rect.x + rect.width / 2) - scrollDom.scrollLeft,
        y: el.getAttribute('yoffset')
            ? el.getAttribute('yoffset') === 'bottom'
                ? normY(
                      rect.top +
                          scrollDom.scrollTop +
                          rect.height / 2 -
                          (scrollDom.scrollHeight - window.innerHeight)
                  )
                : normY(rect.top + rect.height / 2 - keyframe)
            : 0,
        size:
            rect.width > rect.height ? rect.height * scale : rect.width * scale,
        h: rect.height,
        w: rect.width,
        keyframe: keyframe,
        ...attributes,
...
const mapClamp = (value, inMin, inMax, outMin, outMax) =>
    Math.min(
        Math.max(
